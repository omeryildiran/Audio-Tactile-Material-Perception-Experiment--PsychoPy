...
expName = 'AudioTactile'  # from the Builder filename that created this script
expInfo = {'participant': ''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\omeru\\Documents\\FND\\My Repos\\Audio-Tactile Material Perception\\AudioTactile_lastrun.py',
...
wait_2 = ''
wait_2Container = []
sounnoised = sound.Sound('noise.wav', secs=1.4, stereo=True, hamming=True,
    name='sounnoised')
...
audio_num = visual.TextStim(win=win, name='audio_num',
    text='',
    font='Open Sans',
...
SoundTrials = data.TrialHandler(nReps=4.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('aud_tac.csv'),
...
for thisSoundTrial in SoundTrials:
    ...
    # update component parameters for each repeat
    sound_1.setSound(soundsFile, secs=19.0, hamming=True)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if sound_1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sound_1.tStartRefresh + 19.0-frameTolerance:
                # keep track of stop time/frame for later
    ...
    sounnoised.setSound('noise.wav', secs=1.4, hamming=True)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if sounnoised.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sounnoised.tStartRefresh + 1.4-frameTolerance:
                # keep track of stop time/frame for later
    ...
    continueRoutine = True
    # update component parameters for each repeat
    audio_num.setText(audioNum)
    # keep track of which components have finished
    blankComponents = [next_button, audio_num]
    ...
    while continueRoutine:
        ...
        # *audio_num* updates
        if audio_num.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            audio_num.frameNStart = frameN  # exact frame index
            audio_num.tStart = t  # local t and not account for scr refresh
            audio_num.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(audio_num, 'tStartRefresh')  # time at next scr refresh
            audio_num.setAutoDraw(True)
    ...
    for thisComponent in blankComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    SoundTrials.addData('audio_num.started', audio_num.tStartRefresh)
    SoundTrials.addData('audio_num.stopped', audio_num.tStopRefresh)
    audioNum=audioNum+1
    audioNum=audioNum%4
