...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\omeru\\Documents\\FND\\My Repos\\Audio-Tactile Material Perception\\Auditory_lastrun.py',
...
end_text = visual.TextStim(win=win, name='end_text',
    text='Deneyimizin ilk aşaması bitmiştir. Diğer aşamaya geçmek için araştırmacı ile irtibata geçiniz.',
    font='Open Sans',
...
SoundTrials = data.TrialHandler(nReps=1.0, method='random', 
...
for thisSoundTrial in SoundTrials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if BeepSound.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
    ...
    # update component parameters for each repeat
    sound_1.setSound(soundsFile, secs=1, hamming=True)
    ...
    while continueRoutine:
        ...
        leftAdjList=['Granular','Moist','Flexible','Hard','Durable','Rough','Fluffy','Hot','Cheap','Natural']
        rightAdjList=['Whole','Dry','Inflexible','Soft','Flimsy','Smooth','Flattened','Cold','Expensive','Synthetic']
        for i in range(10):
            #k=(i+b)%10
            k=posListe[i]
